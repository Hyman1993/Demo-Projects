...
$(function() {
	...
	$(".blog-content-container").on("click","#submitComment", function () { 
		...
		var csrfToken = $("meta[name='_csrf']").attr("content");
		var csrfHeader = $("meta[name='_csrf_header']").attr("content");
 		var ISLOGIN = $("#islogin").length > 0;//判断是否登录
		$.ajax({ 
			 url: '/comments', 
			 type: 'POST', 
			 data:{"blogId":blogId, "commentContent":$('#commentContent').val(), "isLogin":ISLOGIN},
			 beforeSend: function(request) {
                 request.setRequestHeader(csrfHeader, csrfToken); // 添加  CSRF Token 
             },
			 success: function(data){
	...
	$(".blog-content-container").on("click","#submitVote", function () { 
		...
		var isLogin = $("#islogin").length > 0;//判断是否登录
		// 获取当前用户的ip
        if(!isLogin){ // 获取游客ip，用的是搜狐的接口
          var  ip = returnCitySN["cip"];
        }
		$.ajax({ 
				 url: '/votes', 
				 type: 'POST', 
				 data:{"blogId":blogId,"isLogin":isLogin,"ip":ip},
				 beforeSend: function(request) {
	                 request.setRequestHeader(csrfHeader, csrfToken); // 添加  CSRF Token
	             },
				 success: function(data){
